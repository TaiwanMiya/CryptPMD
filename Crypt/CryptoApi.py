...
from Crypt.Canvas import Printf
import os
import enum
import binascii
import tempfile
...
class File:
    ...
    def read(self,method = 'rb') -> bytes:
        with open(self.path,method) as reader:
            data = reader.read()
            reader.close()
            return data

    def write(self,method = 'wb') -> None:
        if not self.value:
            raise ValueError('Writing a file requires a string')
        if isinstance(self.value,str):
            self.value = self.value.encode('utf-8')
        with open(self.path,method) as writer:
...
class CryptoHash:
    ...
    algorithms = set(('md5', 'sha1', 'sha224', 'sha256', 'sha384', 'sha512'))

    class Hash(object):

        def __init__(self,salt=b'') -> None:
            self.salt = salt
            if isinstance(self.salt,bytes):
                self.salt = self.salt
            elif isinstance(self.salt,str):
        ...
        def MD5(self):
            self.refresh = hashlib.md5(self.salt)
            self.digest = self.refresh.digest()
            self.hexdigest = self.refresh.hexdigest()
            self.size = (self.refresh.block_size,self.refresh.digest_size)
            return self.refresh

        def SHA1(self):
            self.refresh = hashlib.sha1(self.salt)
            self.digest = self.refresh.digest()
            self.hexdigest = self.refresh.hexdigest()
            self.size = (self.refresh.block_size,self.refresh.digest_size)
            return self.refresh
        ...
        def SHA224(self):
            self.refresh = hashlib.sha224(self.salt)
            self.digest = self.refresh.digest()
            self.hexdigest = self.refresh.hexdigest()
            self.size = (self.refresh.block_size,self.refresh.digest_size)
            return self.refresh
        ...
        def SHA256(self):
            self.refresh = hashlib.sha256(self.salt)
            self.digest = self.refresh.digest()
            self.hexdigest = self.refresh.hexdigest()
            self.size = (self.refresh.block_size,self.refresh.digest_size)
            return self.refresh
        ...
        def SHA384(self):
            self.refresh = hashlib.sha384(self.salt)
            self.digest = self.refresh.digest()
            self.hexdigest = self.refresh.hexdigest()
            self.size = (self.refresh.block_size,self.refresh.digest_size)
            return self.refresh
        ...
        def SHA512(self):
            self.refresh = hashlib.sha512(self.salt)
            self.digest = self.refresh.digest()
            self.hexdigest = self.refresh.hexdigest()
            self.size = (self.refresh.block_size,self.refresh.digest_size)
            return self.refresh
