...
namespace Crypto.CryptoItems
{
    public partial class AES : UserControl
    {
        ...
        private void ComboBoxLoad()
        {
            ...
            for (int i = 0; i < AES_Format.Items.Length; i++)
            {
                FileTypeItems.Items.Add(AES_Format.Items[i]);
            }
            Cryption = new RadioButton[]{ CTR, CBC, CFB, OFB, ECB};
        ...
        {
            int[] KeyLength = new int[] { 16, 24, 32 };
        ...
        private void Open_Click(object sender, EventArgs e)
        {
            ...
            foreach (var info in ReadWrite.OpenTheFile(AES_Format.Path[FileTypeItems.SelectedIndex], name))
            {
        ...
        {
            string name = Enum.GetName(typeof(AESname), 0x01 << counter);
            int[] NeedIV = new int[] { 1 , 2 , 3 };
            int[] NeedPadding = new int[] { 1 , 4 };
            if (!IsEncrypt)
            {
            ...
            if (!ReadWrite.CryptionOfOpen(ReadWrite.GetAesRoute[0], $"{name} Keys", 5)) // key路徑
            {
                return;
            }
            if (NeedIV.Contains(counter))
            {
                if (!ReadWrite.CryptionOfOpen(ReadWrite.GetAesRoute[1], $"{name} IV", 6)) // iv路徑
                {
                    return;
                }
            }
            if (counter == 2)
            {
                bool HaveSize = IsEncrypt 
                    ? EncryptSizeCFB.Text != string.Empty 
                    : DecryptSizeCFB.Text != string.Empty;
                if (!HaveSize)
                {
                    Terminal.ErrorMessage("CFB需要輸入Segment! 段大小");
                    return;
                }
                else
                {
                    string Segment = IsEncrypt
                        ? EncryptSizeCFB.Text
                        : DecryptSizeCFB.Text;
                    Format.CryptItems[9] = Segment;
                }
            }
        ...
        private void Encrypt_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Cryption.Length; i++)
            {
                ...
                {
                    counteres = i;
                    path = ReadWrite.GetAesEnCryptRoute[i];
                    break;
                }
            }
