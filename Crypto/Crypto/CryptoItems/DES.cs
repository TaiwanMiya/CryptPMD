...
namespace Crypto.CryptoItems
{
    public partial class DES : UserControl
    {
        ...
        private void ComboBoxLoad()
        {
            ...
            for (int i = 0; i < DES_Format.Items.Length; i++)
            {
                FileTypeItems.Items.Add(DES_Format.Items[i]);
            }
        ...
        private void Open_Click(object sender, EventArgs e)
        {
            ...
            foreach (var info in ReadWrite.OpenTheFile(DES_Format.Path[FileTypeItems.SelectedIndex], name))
            {
        ...
        private void DealWith(string path, int counter, bool IsEncrypt)
        {
            ...
            if (!ReadWrite.CryptionOfOpen(ReadWrite.GetDesRoute[0], $"{name} Keys", 5)) // key路徑
            {
                return;
            }
            if (NeedIV.Contains(counter))
            {
                if (!ReadWrite.CryptionOfOpen(ReadWrite.GetDesRoute[1], $"{name} IV", 6)) // iv路徑
                {
                    return;
                }
            }
        ...
        private void Encrypt_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Cryption.Length; i++)
            {
                ...
                {
                    counteres = i;
                    path = ReadWrite.GetDesEnCryptRoute[i];
                    break;
                }
            }
